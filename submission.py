    ...
    def observe(self, agentX: int, agentY: int, observedDist: float) -> None:
        ...
        for col in range(self.belief.getNumCols()):
            for row in range(self.belief.getNumRows()):
                ...
                currP = self.belief.getProb(row, col)
                emisP = util.pdf(dist, Const.SONAR_STD, observedDist)
                newP = currP * emisP
                self.belief.setProb(row, col, newP)
...
class ParticleFilter(object):
    ...
    def observe(self, agentX: int, agentY: int, observedDist: float) -> None:
        ...
        # raise Exception("Not implemented yet")
        proposal = collections.defaultdict(float)
        for (row, col), numOfParticles in self.particles.items():
            x, y = util.colToX(col), util.rowToY(row)
            dist = math.sqrt((x-agentX)**2 + (y-agentY)**2)
            emisP = util.pdf(dist, Const.SONAR_STD, observedDist)
            proposal[(row, col)] = numOfParticles * emisP

        newParticles = collections.defaultdict(int)
        for _ in range(self.NUM_PARTICLES):
            particle = util.weightedRandomChoice(proposal)
    ...
    def elapseTime(self) -> None:
        ...
        for (row, col), numofParticles in self.particles.items():
            for _ in range(numofParticles):
                particle = util.weightedRandomChoice(self.transProbDict[(row, col)])
                newParticles[particle] += 1
